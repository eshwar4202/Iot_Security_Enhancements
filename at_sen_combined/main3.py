...
def get_metrics():
    """Serves aligned sensor and attack data for Grafana with no null values."""
    ...
    # Build aligned datapoints with 0 for missing values
    lane_1_datapoints = []
    lane_2_datapoints = []
    attack_datapoints = []

    for ts in sorted_timestamps:
        lane_1_value = lane_1_data.get(ts, 0)  # 0 if no data
        lane_2_value = lane_2_data.get(ts, 0)  # 0 if no data
        attack_value = attack_data_points.get(ts, 0)  # 0 if no attack
